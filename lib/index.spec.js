...
describe('Throw error', () => {
  ...
  it('Default guid path is working', (done) => {
    const argsDef = [
      {
        name : 'errorCode',
        order: 1
    ...
    const errData = {'key': 'val2'};
    const error = SevenBoom.badRequest('my message', errData, 'myErrCode');
    try {
      throwError(error, null);
    } catch(err) {
      const finalError = formatError(err);
      expect(err.message).to.equal(finalError.guid);
    }
    done();
  });

  it('Changing guid path is working', (done) => {
    const argsDef = [
      {
        name : 'errorCode',
        order: 1
    ...
    const formatError = formatErrorGenerator();
    const errData = {'key': 'val2'};
    const error = SevenBoom.badRequest('my message', errData, 'myErrCode', '123guid');

    try {
      throwError(error, null, 'customGuid');
    } catch(err) {
      const finalError = formatError(err);
      expect(err.message).to.equal('123guid');
      expect(err.message).to.equal(finalError.customGuid);
    }
    done();
  });

  it('Guid is working when guid path not exist', (done) => {
    const argsDef = [
      {
        name : 'errorCode',
        order: 1
    ...
    try {
      ...
      const finalError = formatError(err);
      expect(err.message).to.match(/^[0-9A-F]{8}-[0-9A-F]{4}-4[0-9A-F]{3}-[89AB][0-9A-F]{3}-[0-9A-F]{12}$/i);
      expect(err.message).to.equal(finalError.customGuid);
    }
  ...
  it('Default guid is working when there is no guid key', (done) => {
    const argsDef = [
      {
        name : 'errorCode',
        order: 1
...
describe('Format error', () => {
  ...
  it('Transform regular error thrown by throwError to SevenBoom error', (done) => {
    const formatError = formatErrorGenerator();
  ...
  it('Transform regular error thrown by native throw to SevenBoom error', (done) => {
    const formatError = formatErrorGenerator();
    const err = new Error('my message');
    try {
      throw(err);
  ...
  it('Transform not found (in map) error to SevenBoom error', (done) => {
    let formatErrorGenerator = index.__get__("formatErrorGenerator");
    const formatError = formatErrorGenerator();
    const origErr = new Error('my orig message');
    try {
      throwError(origErr);
    } catch(err) {
      let errorMap = index.__get__("errorMap");
      errorMap.clear();
      const finalError = formatError(origErr);
      expect( finalError.statusCode ).to.equal(500);
      expect( finalError.message ).to.equal('An internal server error occurred');
    }
    done();
